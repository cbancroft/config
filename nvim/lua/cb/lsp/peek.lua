...
local function create_floating_file(location, opts)
  ...
  local contents = vim.api.nvim_buf_get_lines(
    ...
    range.start.line,
    math.min(
      range['end'].line + 1 + (opts.context or cb.lsp.peek.max_height),
      range.start.line + (opts.max_height or cb.lsp.peek.max_height)
    ),
    false
  )
  ...
  opts = vim.lsp.util.make_floating_popup_options(if_nil(width, cb.lsp.peek.max_width), if_nil(height, cb.lsp.peek.max_height), opts)
  opts['style'] = nil
  ...
  -- Set autocommands to close the window
  vim.api.nvim_command(
    string.format('autocmd %s <buffer> ++once lua pcall(vim.api.nvim_win_close, %d, true)', table.unpack(close_events), winnr)
  )
...
local function preview_location_callback_passthru(_, result)
  return preview_location_callback(result)
end
function M.open_file()
  -- Get the file open in the floating window
...
function M.Peek(what)
  ...
  if vim.tbl_contains(vim.api.nvim_list_wins(), M.floating_win) then
    ...
    vim.api.nvim_buf_set_keymap(
      ...
      'n',
      '<cr>',
      ':lua require "cb.lsp.peek".open_file()<CR>',
      {noremap = true, silent = true}
    )
  else
    -- Make a new request and then create window in callback
    local params = vim.lsp.util.make_position_params()
    local preview_callback = preview_location_callback_passthru
    local success, _ = pcall(vim.lsp.buf_request, 0, 'textDocument/' .. what, params, preview_callback)
    if not success then
      vim.notify(
        'peek: Error calling LSP method "textDocument/' .. what .. '". The current LSP may not support it.',
