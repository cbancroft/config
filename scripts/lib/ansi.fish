...
set ANSI_ESC $'\033'
set ANSI_CSI "${ANSI_ESC}["
set ANSI_OSC "${ANSI_ESC}]"
set ANSI_ST "${ANSI_ESC}\\"
set ANSI_REPORT "" # The return value from ansi::report

function ansi::backward
...
function ansi::blueIntense
    printf '%s94m' "$ANSI_CSI"
end

function function ansi::bgBlack
    printf '%s40m' "$ANSI_CSI"
end

function ansi::bgBlackIntense
    printf '%s100m' "$ANSI_CSI"
end

function ansi::bgBlue
    printf '%s44m' "$ANSI_CSI"
end

function ansi::bgBlueIntense
    printf '%s104m' "$ANSI_CSI"
end

function ansi::bgColor
    printf '%s48;5;%sm' "$ANSI_CSI" "$1"
end

function ansi::bgCyan
    printf '%s46m' "$ANSI_CSI"
end

function ansi::bgCyanIntense
    printf '%s106m' "$ANSI_CSI"
end

function ansi::bgGreen
    printf '%s42m' "$ANSI_CSI"
end

function ansi::bgGreenIntense
    printf '%s102m' "$ANSI_CSI"
end

function ansi::bgMagenta
    printf '%s45m' "$ANSI_CSI"
end

function ansi::bgMagentaIntense
...
function ansi::bgRedIntense
    printf '%s101m' "$ANSI_CSI"
end

function ansi::bgWhite
    printf '%s47m' "$ANSI_CSI"
end

function ansi::bgWhiteIntense
    printf '%s107m' "$ANSI_CSI"
end

function ansi::bgYellow
    printf '%s43m' "$ANSI_CSI"
end

function ansi::bgYellowIntense
...
function ansi::colorCodes
    ...
    for code in 8 9 10 11 12 13 14 15
        if "$code" = 9
            ansi::black
end
        ansi::colorCodePatch "$code"
    ...
    for code in 232 233 234 235 236 237 238 239 240 241 242 243
        ansi::colorCodePatch "$code"
end
    ...
    for code in 244 245 246 247 248 249 250 251 252 253 254 255
        ansi::colorCodePatch "$code"
	end
...
function ansi::colorCodePatch
    ansi::bgColor "$1"
    printf ' %3s ' "$1"
    ansi::resetBackground
end

function ansi::colorTable
    ...
    while read -r colorLabel functionName resetFunction
        printf -- '--%s ' "$colorLabel"
        $functionName
        printf 'Sample'
        $resetFunction
...
function ansi::colorTableLine
    set fn ''

    printf '%-12s' "$1"
    for fn in ansi::black ansi::red ansi::green ansi::yellow ansi::blue ansi::magenta ansi::cyan ansi::white
        $2
        ${fn}
        printf '%s' "$3"
        ${fn}Intense
...
function ansi::column
    printf '%s%sG' "$ANSI_CSI" "$1"
end

function ansi::columnRelative
...
function ansi::cyanIntense
    printf '%s96m' "$ANSI_CSI"
end

function ansi::deleteChars
    printf '%s%sP' "$ANSI_CSI" "$1"
end

function ansi::deleteLines
    printf '%s%sM' "$ANSI_CSI" "$1"
end

function ansi::doubleUnderline
...
function ansi::encircle
    printf '%s52m' "$ANSI_CSI"
end

function ansi::eraseDisplay
    printf '%s%sJ' "$ANSI_CSI" "$1"
end

function ansi::eraseChars
    printf '%s%sX' "$ANSI_CSI" "$1"
end

function ansi::eraseLine
...
function ansi::font
    printf '%s1%sm' "$ANSI_CSI" "$1"
end

function ansi::forward
    printf '%s%sC' "$ANSI_CSI" "$1"
end

function ansi::fraktur
...
function ansi::ideogramLeft
    printf '%s62m' "$ANSI_CSI"
end

function ansi::ideogramLeftDouble
    printf '%s63m' "$ANSI_CSI"
end

function ansi::ideogramRight
    printf '%s60m' "$ANSI_CSI"
end

function ansi::ideogramRightDouble
    printf '%s61m' "$ANSI_CSI"
end

function ansi::ideogramStress
    printf '%s64m' "$ANSI_CSI"
end

function ansi::insertChars
    printf '%s%s@' "$ANSI_CSI" "$1"
end

function ansi::insertLines
...
function ansi::isAnsiSupported
    ...
    if test -n "$ANSI_FORCE_SUPPORT"
        return 0
end

    if hash tput &> /dev/null
        if test "$(tput colors)" -lt 8
            return 1
        fi
...
function ansi::magentaIntense
    printf '%s95m' "$ANSI_CSI"
end

function ansi::nextLine
    printf '%s%sE' "$ANSI_CSI" "$1"
end

function ansi::noBlink
    printf '%s25m' "$ANSI_CSI"
end

function ansi::noBorder
    printf '%s54m' "$ANSI_CSI"
end

function ansi::noInverse
    printf '%s27m' "$ANSI_CSI"
end

function ansi::normal
    printf '%s22m' "$ANSI_CSI"
end

function ansi::noOverline
    printf '%s55m' "$ANSI_CSI"
end

function ansi::noStrike
    printf '%s29m' "$ANSI_CSI"
end

function ansi::noUnderline
    printf '%s24m' "$ANSI_CSI"
end

function ansi::overline
...
function ansi::position
	local position=(string replace ',' ';', $1)
    printf '%s%sH' "$ANSI_CSI" "$position"
end

function ansi::previousLine
    printf '%s%sF' "$ANSI_CSI" "$1"
end

function ansi::rapidBlink
    printf '%s6m' "$ANSI_CSI"
end
...
function ansi::redIntense
    printf '%s91m' "$ANSI_CSI"
end

function ansi::repeat
    printf '%s%sb' "$ANSI_CSI" "$1"
end

function ansi::report
    local buff c report
    ...
    set report ""
read -r -N "$(count $2)" -s -t 1 buff

    if "$buff" != "$2"
        return 1
end

read -r -N "$(count $3)" -s -t 1 buff

    while "$buff" != "$3"
set report "$report$(string sub -l 1 $buff)"
        read -r -N 1 -s -t 1 c || exit 1
		set buff "$(string sub -s 1 $buff)$c"
end

    ANSI_REPORT=$report
end

function ansi::reportPosition
    ansi::report 6n "$ANSI_CSI" R || return 1
    printf '%s\n' "${ANSI_REPORT//;/,}"
end

function ansi::reportIcon
    ansi::report 20t "${ANSI_OSC}L" "$ANSI_ST" || return 1
    printf '%s\n' "${ANSI_REPORT//;/,}"
end

function ansi::reportScreenChars
    ansi::report 19t "${ANSI_CSI}9;" t || return 1
    printf '%s\n' "${ANSI_REPORT//;/,}"
end

function ansi::reportTitle
    ansi::report 21t "${ANSI_OSC}l" "$ANSI_ST" || return 1
    printf '%s\n' "${ANSI_REPORT//;/,}"
end

function ansi::reportWindowChars
    ansi::report 18t "${ANSI_CSI}8;" t || return 1
    printf '%s\n' "${ANSI_REPORT//;/,}"
end

function ansi::reportWindowPixels
    ansi::report 14t "${ANSI_CSI}4;" t || return 1
    printf '%s\n' "${ANSI_REPORT//;/,}"
end

function ansi::reportWindowPosition
    ansi::report 13t "${ANSI_CSI}3;" t || return 1
    printf '%s\n' "${ANSI_REPORT//;/,}"
end

function ansi::reportWindowState
...
function ansi::resetBackground
    printf '%s49m' "$ANSI_CSI"
end

function ansi::resetColor
    printf '%s0m' "$ANSI_CSI"
end

function ansi::resetFont
    printf '%s10m' "$ANSI_CSI"
end

function ansi::resetForeground
    printf '%s39m' "$ANSI_CSI"
end

function ansi::resetIdeogram
    printf '%s65m' "$ANSI_CSI"
end

function ansi::restoreCursor
    printf '%su' "$ANSI_CSI"
end
...
function ansi::saveCursor
    printf '%ss' "$ANSI_CSI"
end

function ansi::scrollDown
    printf '%s%sT' "$ANSI_CSI" "${1-}"
end

function ansi::scrollUp
...
function ansi::strike
    printf '%s9m' "$ANSI_CSI"
end

function ansi::tabBackward
    printf '%s%sZ' "$ANSI_CSI" "${1-}"
end

function ansi::tabForward
