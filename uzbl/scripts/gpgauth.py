...
def getText(authInfo, authHost, authRealm):
    ...
    saveInfo = gtk.CheckButton( label="Save Authentication Information" )

    keyring = pwh.Keyring( "uzbl", authHost + "@" + authRealm, "auth" )
    if( keyring.has_credentials() ):
        (uname,passwd) = keyring.get_credentials()

        if uname:
            login.set_text( uname )

        if passwd:
            password.set_text( passwd )
    ...
    dialog.vbox.pack_end(saveInfo)
    dialog.show_all()
    rv = dialog.run()

    output = login.get_text() + "\n" + password.get_text()

    if saveInfo.get_active():
        keyring.set_credentials((login.get_text(),password.get_text()))
