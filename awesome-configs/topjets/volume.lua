...
function volume.inc(w)
   update_volume(w, io.popen("amixer -D pulse set Master 5%+"), true)
end
...
function volume.mute(w)
   update_volume(w, io.popen("amixer -D pulse set Master toggle"), true)
end
...
function volume.new()
   ...
   scheduler.register_recurring("topjets_volume", 20,
                                function()
                                   update_volume(w, io.popen("amixer -D pulse get Master"), false)
                                end)
   return w
end
