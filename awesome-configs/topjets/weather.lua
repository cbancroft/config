...
local weather = base {
   cmd = "curl -s 'https://api.forecast.io/forecast/%s/%s,%s?units=us&exclude=minutely,hourly,alerts,flags'",
...
local function forecast_line (t, today)
   ...
   local day, temp
   if today then
      day, temp = "Today", format("%d(%d)°F", utility.round(t.temperature),
                                  utility.round(t.apparentTemperature))
   else
      day, temp = os.date("%d.%m", t.time), format("%d/%d°F", utility.round(t.temperatureMin),
...
function weather.callback(f)
   ...
   weather.refresh_all(format("%d°F", utility.round(weather.data.currently.temperature)),
